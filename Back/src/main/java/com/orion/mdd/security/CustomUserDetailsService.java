...
public class CustomUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String userNameOrPassword) throws UsernameNotFoundException {
        List<User> user = userRepository.findByUsernameOrEmail(userNameOrPassword, userNameOrPassword);
        if (user.size() != 1) {
            throw new UsernameNotFoundException("User not found with Username or Email: " + userNameOrPassword);
        }
